...
const GameAccountManager = () => {
  ...
  const handleDeleteGameAccount = async () => {
    ...
    try {
      const res = await fetchWithAuth(`${weburl}/api/gameaccount/${selectedAccount.JID}/request-deletion`, {
        method: 'POST',
      });
      ...
      if (res.ok) {
        ...
        toast({
          ...
          description: `A confirmation email has been sent to your email address. Please check your inbox and click the confirmation link to permanently delete the game account \"${selectedAccount.StrUserID}\".`,
        });
      } else {
        const data = await res.json();
        toast({
      ...
      {gameAccounts.length > 0 && (
        <Card className='bg-lafftale-darkgray border-lafftale-gold/30'>
          ...
          <CardContent className='space-y-6'>
            ...
            <Select
              value={selectedAccount?.JID.toString()}
              onValueChange={(jid) => {
                      ...
                      <AlertDialogContent className='bg-lafftale-darkgray border-lafftale-gold'>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            ...
                            progress will be permanently lost after confirmation.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
