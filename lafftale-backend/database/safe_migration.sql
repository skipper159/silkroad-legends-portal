-- =====================================================
-- SAFE REFERRAL ANTI-CHEAT & DELAYED REWARDS MIGRATION
-- =====================================================
-- This script executes updates in a safe manner without column dependency issues

USE SRO_CMS;

PRINT '🚀 Starting Safe Referral Anti-Cheat & Delayed Rewards Migration...';

-- =====================================================
-- PHASE 1: BACKUP & LIFETIME SETTINGS
-- =====================================================
...
BEGIN
    ...
    PRINT '✅ Updated IP settings to lifetime';
END
...
BEGIN
    ...
    PRINT '✅ Created IP lifetime setting';
END;

-- Update Fingerprint lifetime settings
...
BEGIN
    ...
    PRINT '✅ Updated fingerprint settings to lifetime';
END
...
BEGIN
    ...
    PRINT '✅ Created fingerprint lifetime setting';
END;

-- Add blocking settings
...
BEGIN
    ...
    PRINT '✅ Added IP blocking setting';
END;
...
BEGIN
    ...
    PRINT '✅ Added fingerprint blocking setting';
END;

-- =====================================================
-- PHASE 2: ADD NEW COLUMNS TO REFERRALS TABLE
-- =====================================================

PRINT '📋 Adding new columns to referrals table...';
...
-- Add requires_validation column
...
BEGIN
    ...
    PRINT '✅ Added requires_validation column';
END;

-- Add reward_given_at column
...
BEGIN
    ...
    PRINT '✅ Added reward_given_at column';
END;

-- Add cheat_reason column
...
BEGIN
    ...
    PRINT '✅ Added cheat_reason column';
END;
...
-- Add validation and reward tracking columns
...
PRINT '📋 Creating Anti-Cheat tables...';
...
BEGIN
    ...
    PRINT '✅ Created referral_anticheat_logs table';
END;
...
-- =====================================================
-- PHASE 4: ADD ALL SETTINGS
-- =====================================================

PRINT '📋 Adding Anti-Cheat & Delayed Reward settings...';
...
) AS NewSettings(setting_key, setting_value, description, created_at, updated_at)
WHERE NOT EXISTS (
    SELECT 1 FROM referral_settings 
    WHERE referral_settings.setting_key = NewSettings.setting_key
);

PRINT '✅ Added all Anti-Cheat & Delayed Reward settings';
...
) AS TestVPNs(ip_address, provider, country_code, confidence)
WHERE NOT EXISTS (
    SELECT 1 FROM known_vpn_ips 
    WHERE known_vpn_ips.ip_address = TestVPNs.ip_address
);
...
-- =====================================================
-- PHASE 6: UPDATE EXISTING DATA (SAFE)
-- =====================================================

PRINT '📋 Safely updating existing referral data...';

-- Update existing referrals with new status system
-- Only update if status column exists and avoid circular references
UPDATE referrals 
SET status = CASE 
...
PRINT '✅ Updated ' + CAST(@updated_count AS NVARCHAR(10)) + ' existing referrals';
...
DECLARE @settings_count INT, @pending_count INT, @active_count INT, @rejected_count INT, @tables_count INT;
...
PRINT '  🗄️ New Tables Created: ' + CAST(@tables_count AS NVARCHAR(10));
PRINT '  📈 Referral Status Distribution:';
...
PRINT '  ✅ Lifetime IP/Fingerprint blocking';
